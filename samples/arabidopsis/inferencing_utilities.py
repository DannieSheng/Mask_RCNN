...
class ImgInstSegments:
    ...
    def segmentation_quality_control(self):
        ...
        if len(np.where(counts > 1)[
                   0]):  # if there are more than one mask that heavily overlap the same mask (with IOS > 0.5)
            if counts.max() > 2:
                print('Warning: Issue type {}: not handled! \n'.format(type_iss))
            else:
    ...
    def visualize_whole(self, updated=False, colors=None, print_img=False, savename=None):
        ...
        visualize.display_instances(image_, r['rois'], r['masks'], r['class_ids'], self.class_names, r['scores'],
    ...
    def visualize_separate(self, updated=False, colors=None, print_img=False, savepath=None):
        ...
        for idx in range(0, num_instances):
            ...
            visualize.display_instances(image, roi_i, mask_i, class_id_i, self.class_names, score_i,
                                        ax=_get_ax(rows=1, cols=1, size=16), show_bbox=True, show_mask=True,
    ...
    def save_self(self, savename, mode='result'):
        ...
        if mode == 'all':
            ...
            pkl.dump(vars(self), open(savename, 'wb'))
        else:
            delattr(self, 'image')
            # pkl.dump(self, open(savename,'wb'))
            pkl.dump(vars(self), open(savename, 'wb'))
...
#                  name_model='model.h5', list_files=None):
#
...
#             self.ext = self.suffix
#         else:
#             self.ext = self.suffix.split(".")[-1]
#
#         self.class_names = class_names
#
...
#
#         # initialize ImgInstSegments class
#         img_inst_segs = ImgInstSegments(image=img, class_names=self.class_names)
...
#         print(
#             'There were {} instances segmented in {}, {} were left after quality control.\n'.format(num_inst, file_name,
#                                                                                                     num_inst_final))
...
#             img_inst_segs.save_self(savename=os.path.join(self.savedir, filename.replace(self.ext, 'pkl')),
#                                     mode='result')
#             if count == 1:
...
#         csvfile.close()
#         print(
#             'Done! \nThe results are saved here: {}. \nThe visualization are saved here: {}.\nThe updated visualization are saved here: {}'.format(
#                 self.savedir, self.visualization_dir, self.updated_dir))
